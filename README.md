...
BRAVO is now available directly through **PyPI** and can be installed using the standard Python package manager

### Install the latest supported release

The following process will work for all system because it is pure Python package. Additional dependencies such as MySQL are individual system-specific. 

```bash
pip install BRAVOServer==3.1.1

# Create Initial Environment File
BRAVO-Initialization

# Start BRAVO Application
BRAVO-RunServer
```

User then can access `localhost:27286` for UI access
